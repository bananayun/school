...
public class EmailUtils {
	public static void sendEmail(User user){
		//邮箱
		String myAccount = "bananamoll@163.com";
		//授权码
		String myPass = "RJUNSTDKWDMYWRLB";
		//邮箱服务器
	...
	private static MimeMessage createMsg(Session session, String myAccount, User user) {
		...
		try {
			//3.1发送方
			message.setFrom(new InternetAddress(myAccount, "banana网官方邮件", "utf-8"));
			//3.2设置接收方
			/*
			...
			//3.3 设置主题
			message.setSubject("banana网激活码","utf-8");
			//获取本机的ip地址
			String ip = Inet4Address.getLocalHost().getHostAddress();
			InetAddress addr = InetAddress.getLocalHost();
			String url = "http://"+ip+":8080/banana/activate?e="+ Base64Utils.encode(user.getEmail())+"&c="+Base64Utils.encode(user.getActivatecode());
			//设置正文信息
			message.setContent(user.getUsername()+",欢迎你("+addr.getHostName()+")加入我们<br>为了更好体验我们的产品，请<a href='"+url+"'>点击激活 "+url+"</a>","text/html;charset=utf-8");
			//设置日期
