<%@page import="cn.itcast.itcaststore.domain.PageBean" %>
...
<body class="main">
	...
	<%
		PageBean a = (PageBean)request.getAttribute("bean");
		String strs = a.getCategory();
	%>
	<%!
									...
									<ul>
										<c:if test="${bean.currentPage!=1}">
											<li class="disablepage_p">
												<a class="disablepage_a" href="${pageContext.request.contextPath}/showProductByPage?currentPage=${bean.currentPage-1}&category=<%=convert(strs) %>"></a>
											</li>
										</c:if>
										<c:if test="${bean.currentPage==1}">
										...
										<c:forEach begin="1" end="${bean.totalPage}" var="pageNum">
											...
											<c:if test="${pageNum!=bean.currentPage}">
												<li><a href="${pageContext.request.contextPath}/showProductByPage?currentPage=${pageNum}&category=<%=convert(strs) %>">${pageNum}</a>
												</li>
											</c:if>
										</c:forEach>
